...
import pl.edu.icm.unicore.portal.angio.ui.job.AngioJobPanel;

import java.util.Collection;

/**
 * Top level component of AngioMerge interface. The top part contains a jobs
...
public class MainPortalComponent extends CustomComponent
{
			...
			PortalThreadPool tpool, UserProfilesManager profilesMan)
	{
		...
		config = new AngioProperties(configSource.getProperties());
		jobHandler = new JobHandler(new BrokeredJobSubmissionHelper());
		initUI();
	}

	public void setGridEnvironment(AngioGridEnvironment gridEnvironment)
	{
	...
	private void initUI()
	{
		...
		final JobApplicationFilter filter = new JobApplicationFilter(
				AngioMergeJSDLCreator.APPLICATON_NAME);
		final SingleContextAction<AtomicJob> openJob = new SingleContextAction<AtomicJob>(
				msg.getMessage("Angio.MainComponent.openSimulation"), 
		...
		Button newSimulationButton = new Button(
				msg.getMessage("Angio.MainComponent.newSimulationButton"));
		...
		newSimulationButton.addClickListener(new Button.ClickListener()
		{
			...
			{
				AngioJobPanel jobPanel = new AngioJobPanel(config, jobHandler, msg, tpool,
		...
		final Button hideJobsTable = new Button(
				msg.getMessage("Angio.MainComponent.hideJobsTableButton"));
	...
	private void updateShowHideButton(Button hideJobsTable, Component toHide)
	{
		...
		{
			hideJobsTable.setCaption(msg.getMessage("Angio.MainComponent.showJobsTableButton"));
			hideJobsTable.setData(false);
			toHide.setVisible(false);
		} else
		{
			hideJobsTable.setCaption(msg.getMessage("Angio.MainComponent.hideJobsTableButton"));
			hideJobsTable.setData(true);
	...
	{
		AngioJobSpecification jobSpec;
		try
		{
			jobSpec = AngioMergeJSDLCreator.createJobSpecification(job.getProperties());
		} catch (Exception e)
		{
		...
		AngioJobPanel jobPanel = new AngioJobPanel(config, jobSpec, job, jobHandler, msg,
				tpool, Session.getCurrent().getUserGridDiscovery(), gridEnvironment);
		addTab(jobPanel);
	}
	
	private void addTab(AngioJobPanel jobPanel)
	{
