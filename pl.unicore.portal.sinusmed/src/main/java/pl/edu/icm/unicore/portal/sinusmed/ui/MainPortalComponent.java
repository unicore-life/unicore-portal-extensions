...
public class MainPortalComponent extends CustomComponent
{
	...
	private SinusMedGridEnvironment gridEnvironment;
			...
			PortalThreadPool tpool, UserProfilesManager profilesMan)
	{
		...
		config = new SinusMedProperties(configSource.getProperties());
		jobHandler = new JobHandler(new BrokeredJobSubmissionHelper(), new StorageUtil(config, 
				Session.getCurrent().getUser()));
		initUI();
	}

	public void setGridEnvironment(SinusMedGridEnvironment gridEnvironment)
	{
	...
	private void initUI()
	{
		...
		final JobApplicationFilter filter = new JobApplicationFilter(
				SinusMedJSDLCreator.SINUS_APPLICATON_NAME);
		final SingleContextAction<AtomicJob> openJob = new SingleContextAction<AtomicJob>(
				msg.getMessage("SinusMed.MainComponent.openSimulation"), 
