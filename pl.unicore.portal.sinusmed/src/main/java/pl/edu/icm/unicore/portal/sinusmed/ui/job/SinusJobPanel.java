...
import pl.edu.icm.unicore.portal.sinusmed.JobHandler;
 ...
 * @author K. Benedyczak
 */
public class SinusJobPanel extends CustomComponent 
{
	private static final Logger log = Logger.getLogger(SinusJobPanel.class);
	private final SinusMedProperties config;
	private final JobHandler jobHandler;
	private final MessageProvider msg;
	private final SinusMedGridEnvironment gridEnvironment;

	private SinusInputPanel inputPanel;
	...
	public SinusJobPanel(SinusMedProperties config, JobHandler jobHandler, MessageProvider msg,
			PortalThreadPool tpool, UserGridDiscovery discovery, SinusMedGridEnvironment gridEnvironment)
	{
		this.config = config;
	...
	public SinusJobPanel(SinusMedProperties config, SinusMedJobSpecification existingJob, AtomicJob job, 
			JobHandler jobHandler, MessageProvider msg, PortalThreadPool tpool, 
			UserGridDiscovery discovery, SinusMedGridEnvironment gridEnvironment)
	{
	...
	private void init()
	{
		...
		inputPanel = new SinusInputPanel(inputId, config, msg, Session.getCurrent().getUser());
	...
	{
		final SinusMedJobSpecification jobDesc;
		try
		{
	...
	private void doSubmitSynchro(SinusMedJobSpecification jobDesc) throws Exception
	{
		jobMonitoringComponent.setCustomListener(new MyJobStatusListener(UI.getCurrent(), 
				Session.getCurrent().getUser().getCredentials()));
	...
	private void switchToJobMonitoring(SinusMedJobSpecification jobDesc)
	{
	...
	private class MyJobStatusListener implements JobStatusListener
	{
		...
		{
			ui.getSession().lock();
			SinusMedJobSpecification extractedJob;
			try
			{
				extractedJob = SinusMedJSDLCreator.createSinusmedJobSpecification(
						job.getProperties()); 
